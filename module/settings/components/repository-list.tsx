...
export function RepositoryList() {
  ...
  const { data: repositories = [], isLoading } = useQuery({
    ...
    queryFn: async () => await getConnectedRepo(),
    staleTime: 1000 * 60 * 2,
  ...
  const disconnectAllMutation = useMutation({
    ...
    onSuccess: (result) => {
      toast.success(`${result.count} repositories disconnected`);
      queryClient.invalidateQueries({
        queryKey: ["connected-repositories"],
      });
      queryClient.invalidateQueries({
  ...
  if (repositories.length === 0) {
    return (
      <p className="text-sm text-muted-foreground">No connected repositories</p>
    );
  }
